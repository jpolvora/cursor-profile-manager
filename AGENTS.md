...
- **RunProxied profiles** (when Agent Story proxy is running): append `--proxy-server=<url>` and `--ignore-certificate-errors`; launch via `Invoke-ProcessWithEnvironment` with `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` / `GLOBAL_AGENT_*` and `NODE_TLS_REJECT_UNAUTHORIZED=0` so Node agent subprocesses use the MITM proxy; sync `http.proxy` / `http.proxyStrictSSL` / `http.proxySupport` in `<user-data-dir>\User\settings.json`; write `proxy-server` / `proxy-bypass-list` / `ignore-certificate-errors` to `<user-data-dir>\argv.json` for all Electron subprocesses. Proxy URL defaults to `http://127.0.0.1:8080` (`AGENT_STORY_PROXY_URL` overrides). User must **fully quit** existing Cursor windows for that profile before relaunching proxied — proxy env and argv.json apply at process start only.
